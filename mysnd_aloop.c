 ...
 *
 *  Added option to control GPIO pin when playback is active in 2016:
 *  Copyright (c) Tayaout Kuberek <tayaoutlk@gmail.com>
 *
...
static int gpio_delay = 5;
module_param(gpio_num, int, 0444);
MODULE_PARM_DESC(gpio_num, "GPIO pin that is set to high when playback is running.");
module_param(gpio_delay, int, 0444);
MODULE_PARM_DESC(gpio_delay, "GPIO pin delay before set to low when playback stop.");
...
static void gpio_off_func(struct work_struct *unused);
static DECLARE_DELAYED_WORK(work, gpio_off_func);
...
static void 
gpio_off_func(struct work_struct *unused)
{
    if (GPIO_STATE == 0) 
    {
        printk(KERN_INFO "%s: %u\n", __func__, gpio_num);
...
static int loopback_trigger(struct snd_pcm_substream *substream, int cmd)
{
    ...
    case SNDRV_PCM_TRIGGER_STOP:
        ...
        {
            loopback_active_notify(dpcm);
            schedule_delayed_work(&work, HZ * gpio_delay);
            GPIO_STATE = 0;
        }
        break;
...
#define SND_LOOPBACK_DRIVER "mysnd_aloop"
...
static int __init alsa_card_loopback_init(void)
{
   ...
   if (!gpio_is_valid(gpio_num)){
      printk(KERN_INFO "invalid gpio_num: %u\n", gpio_num);
       return -ENODEV;
   }
   gpio_request(gpio_num, "sysfs");  
   gpio_direction_output(gpio_num, GPIO_STATE);
